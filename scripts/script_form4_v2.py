...
for row in ws_input.iter_rows(min_row=2, values_only=True):
    ...
    data_envio = row[4]
    tc_uvr = row[14]
    receita_vendas = row[6]
    receita_servicoes = row[7]
    despesas = row[8]
    material_recilado = row[9]
    rejeito = row[10]
    total_material_processado = row[11]
    postos_de_trabalho = row[12]
    renda_media = row[13]
...
for nome, wb in wb_final.items():
    caminho_saida = pasta_form4 / f"{nome}_atualizado_form4_v2.xlsx"
